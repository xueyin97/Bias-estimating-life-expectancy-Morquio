---
title: "subsample 36 years when life expectancy is constant"
...
```
We subsample the last 36 years under the first simulation scenario when both the mean survival time at 25.3 and the median survival time at 20.8 are constant during the 500 years.
```{r scenario1 36yrs,echo=TRUE}
set.seed(1)
N=500
...
Age.of.death<-weibull_eventtime(25.29704,20.8,0,0,N)
...
#expected mean=25.3
#mean survival time of the 36 years
mean(mean.all)
#expected median=20.8
#median survival time of the 36 years
mean(median.all)
#boxplot of means
...
true<-data.frame(variable=c("mean","median"),hline=c(25.29704,20.8))
df %>% ggplot(aes(x=variable,y=value))+geom_boxplot() +geom_jitter(alpha=0.1,position = position_jitter(width = 0.3,seed=1))+stat_summary(fun.y = "mean", geom = "point", colour = "#000000", size = 2)+ stat_summary(fun.y="mean", colour="#000000", geom="text", vjust=-0.7,size=4, aes( label=round(..y.., digits=1)))+geom_errorbar(data=true,aes(y=NULL,ymax=hline,ymin=hline),linetype="dashed",color=c("#0072B2","#D55E00"),position=position_dodge())+geom_text(aes(1,24.9),label="25.3",size=4,color=c("#0072B2"))+geom_text(aes(2,20.2),label="20.8",size=4,color="#D55E00")+xlab("Statistics")+ylab("Value")+theme_bw(base_size = 18,base_family = "")
```
